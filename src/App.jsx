  ...
  return (
    ...
    <div className={`m-auto flex min-h-screen w-full flex-col items-center justify-start gap-3 px-4 py-4 transition-colors duration-300 sm:px-6 ${isDarkMode ? 'bg-gradient-to-br from-black via-gray-800 to-gray-900' : 'bg-gradient-to-br from-white via-gray-50 to-gray-100'}`}>
      ...
      <p className={`text-sm font-inter font-md transition-colors duration-300 ${isDarkMode ? 'text-white' : 'text-gray-700'}`}>
          IT Intern | UI/UX Designer 
        </p>
        <div className="flex w-full flex-row items-center justify-center gap-3 sm:flex-row sm:items-center sm:justify-start">
          <a href={cvPdf} download="Resume.pdf" target="_blank" rel="noopener noreferrer" className={`text-xs font-inter font-semibold py-2 px-6 rounded-sm transition duration-300 ease-in-out transform hover:scale-105 ${isDarkMode ? 'bg-white text-gray-900 hover:bg-gray-100' : 'bg-black text-white hover:bg-gray-800'}`}>
            Download CV
          </a>
      ...
      <div className="w-full max-w-3xl">
        <div className="grid w-full gap-2 lg:grid-cols-[minmax(300px,0.40fr)_minmax(0,0.6fr)]">
          ...
          <div className={`w-full backdrop-blur-xl border rounded-md px-5 shadow-4xl transition-colors duration-300 ${isDarkMode ? 'bg-white/10 border-white/20' : 'bg-white border-gray-200'}`}>
            ...
            <p className={`w-full max-w-3xl pb-3 text-md font-inter leading-4 text-justify transition-colors duration-300 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>
